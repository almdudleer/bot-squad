...
Opaque, base64url, 32 bytes of entropy. Two prefixes so the wire format
self-describes which kind of bearer the installer is sending:
...
from ``POST /api/m/servers`` and ``POST /api/m/installer/connect``.
