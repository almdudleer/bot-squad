"""Tests for app.task_body — the four-section schema: parse/compose/append.

Mirror of `worker/tests/test_task_body.py`. The MODULES are pinned
byte-identical by the mirror registry; these tests are not, so a behaviour
proved in only one of them is proved on only one import path.
"""
...
def test_parse_non_canonical_sections_are_not_surfaced():
    ...
    body = "## Verbatim request\n\nask\n\n## DoD\n\n- d\n\n## Scope\n\n- s\n"
    out = parse_body(body)
    assert set(out) == set(SECTION_KEYS)  # T-0863 added `summary`
