...
if [ ! -x "$GATE" ]; then
    echo "[staging] FATAL: readiness gate $GATE is missing or not executable." >&2
...
if [ -n "$failed" ]; then
    echo "[staging] FATAL: deploy failed —$failed (app_rc=$app_rc stt_rc=$stt_rc route_rc=$route_rc)" >&2
    echo "[staging] every gate above was RUN; see its own lines for which signal it got." >&2
    # App down is the most severe, then the voice path, then routing.
    [ "$app_rc" -eq 0 ] || exit 22
    [ "$stt_rc" -eq 0 ] || exit 23
    exit 24
fi
