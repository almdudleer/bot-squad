...
- **Every artifact you write has a defined home — no random mds** (T-0567; map: `docs/architecture/D-0045`). Stakeholder/user clarifications, decisions, and answers land on the TICKET the moment they happen (his words → `bsq ticket quote <id>`; new asks → `bsq task new` with verbatim words) — NEVER only in a handover/compact/scratch md. **One task, one artifact: your ticket's `## Context`** (T-0863). You have no compact/handover file — `artifacts/<task_id>.md` and `bsq compact-save` are retired for task-bound sessions and the worker refuses them; `bsq ticket context <id> --file <f>` (which REPLACES the section with what is true now) IS your handoff, and it is what the system waits for when it asks you to finalize. Beside it, `bsq ticket summary <id> "<one paragraph>"` REPLACES `## Executive summary` — the status the STAKEHOLDER reads: progress made and what remains, never a restatement of the ask (T-0863). Scratch (probe scripts, logs, test dumps, Playwright output) goes in your session scratchpad — never the code working tree, never the data dir.
