...
- **The compact is only spent when a resume is certain**, and the SESSION's half of the decision is the **ticket status**: `to_accept` when delivered (T-0944: `totest` is the human's queue — delivery lands in `to_accept`, the operator's acceptance queue, one step before that), `blocked_on_user` when you are waiting on the stakeholder (the system then waits for him instead of restarting work), otherwise leave it and a later session picks the ticket up. That status is literally what the recycler reads next tick — «Это должна решать сама сессия, закончила она работу или нет, но система должна ей ставить дедлайн».
...
- **The handoff has a DEFINED home, and which one turns on whether you own a TASK** (T-0567, T-0863).
  ...
  - **Task-less (the operator, any other role with no assignment) → the role artifact**, system-resolved via `bsq compact-save`: a PROJECT-LEVEL role (operator **or** user-conversation) → the project's ONE work-state doc `artifacts/work-state.md`; any other role → `artifacts/role-<role>-<sid>.md`. Overwrite in place, don't mint variants. **T-0942 merged the first two on purpose**: keyed to the operator ROLE, that doc was written only by a session deriving to `operator`, so a user-session holding the operator seat compacted into a per-role file and the seat's doc sat five weeks stale across 38 spawns. Concurrent holders are arbitrated by a lock + a `rev` CAS (`bsq work-state write --base-rev N`), not by giving each one its own file. **Check its age before you believe it** — `bsq work-state` prints a staleness verdict, and a stale doc is history, not state. These sessions have no ticket to write a Context onto, which is the only reason the artifact seam still exists.
...
This project's worked routing table — every section of `artifacts/work-state.md` (`operator-state.md` before T-0942) classified, with what stays, what is already homed, and where the rest goes — is the runbook `docs/runbook/D-0068`.
