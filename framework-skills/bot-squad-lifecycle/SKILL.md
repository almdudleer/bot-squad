...
- **The handoff has a DEFINED home, and which one turns on whether you own a TASK** (T-0567, T-0863).
  ...
  - **Either way, task-relevant detail belongs on the TASK, not smuggled into a file**: his words → `bsq ticket quote`, a new ask → `bsq task new`, durable state → `bsq ticket context`. An unanswered stakeholder question parked in a handover md is a stranded request. Full kind→home map: project doc `docs/architecture/D-0045`.
