#!/usr/bin/env bash
#
# resume-deploys.sh — remove the PAUSED.json marker so deploy_monitor
# picks the queue back up on its next tick.
#
# Usage (via symlink at ops/bot-squad-bin/resume-deploys):
#   resume-deploys
#
...
if [ -z "$slug" ]; then
    echo "resume-deploys: CWD $PWD not in any registered project" >&2
    exit 2
fi
...
curl -sS --unix-socket "$WORKER_SOCK" \
    ...
    -d "$(python3 -c "import json,sys; print(json.dumps({'slug':sys.argv[1],'requested_by':sys.argv[2]}))" "$slug" "$user")" \
    http://w/actions/resume_deploys
echo
