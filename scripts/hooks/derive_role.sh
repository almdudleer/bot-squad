...
#     2. teamlead/tl window marker (<x>-TL, <x>_teamlead, …)    → teamlead
#     3. default — incl. task-less / marker-less sessions       → dev
#
...
bsq_derive_role() {
    ...
    case "$w_lc" in
        operator|*[-_]operator)            printf 'operator' ;;
        tl|teamlead|*[-_]tl|*[-_]teamlead) printf 'teamlead' ;;
        *)                                 printf 'dev' ;;
    esac
}
