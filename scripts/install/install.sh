...
step_render_env() {
  ...
  # T-0179: at-rest encryption key for secrets.toml (Fernet = urlsafe-base64 of
  # 32 random bytes). Generated like JWT_SECRET so a fresh install encrypts the
  # TG bot token by default. Both the API and worker read it from .env.
  if [[ -z "${BOTSQUAD_SECRETS_KEY:-}" ]]; then
    BOTSQUAD_SECRETS_KEY="$(openssl rand -base64 32 2>/dev/null | tr '+/' '-_' \
      || head -c 32 /dev/urandom | base64 | tr '+/' '-_')"
    log "generated BOT_SQUAD_SECRETS_KEY (Fernet key)"
  fi
...
BOT_SQUAD_SECRETS_KEY=${BOTSQUAD_SECRETS_KEY}
