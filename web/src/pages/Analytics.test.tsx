 ...
 * is swept off Analytics. renderToStaticMarkup never runs effects, so the
 * page's own api.analytics() fetch never fires — no mocking needed, this
 * exercises the same initial synchronous render the browser paints first.
 */
...
describe("Analytics page — onboarding chrome sweep (T-0672)", () => {
  test("renders with no PageHelp toggle/panel", () => {
    const html = renderToStaticMarkup(
      <StaticRouter location="/p/bot-squad/analytics">
        <Analytics />
      </StaticRouter>,
    );
