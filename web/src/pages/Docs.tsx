...
export function Docs() {
  ...
  // Group docs by category for the left rail.
  ...
  async function createDoc() {
    ...
    setFlash(null);
    try {
      const res = await api.createDoc(slug, newCategory, title);
      reload();
      ...
      setShowNew(false);
      setNewTitle("");
      setFlash(`Created ${res.id} in ${res.category} — edit below, then Save.`);
    } catch (e) {
      setError(String(e));
    } finally {
      setBusy(false);
                ...
                {byCategory.get(cat)!.map((d) => (
                  <li key={d.id} className="mb-1">
                    <button
                      ...
                      onClick={() => open(d.id)}
                    >
                      <span style={{ fontFamily: "var(--mc-mono)" }}>{d.id}</span>
                      <br />
