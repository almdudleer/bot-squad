...
def apply(cfg: Any, entry: dict) -> ApplyResult:
    ...
    try:
        ...
        # succession) must not bounce the worker every few minutes either.
        # T-0717: that skip is now DEFERRED, not dropped. It used to rely on "the
        # NEXT successful apply calls this again", which is no guarantee at all
        # when no further release is published — the worker just kept running the
        # pre-update code. The gate records a pending marker and
        # worker_restart_catchup_tick fires it once the window elapses.
        ...
        if _deploy._restart_rate_limited(cfg, source="autoupdate_apply", reason=version):
            log.info(
                ...
                "on-disk code already updated; DEFERRED to the catch-up tick, "
                "which fires it once the window elapses", version,
            )
        else:
