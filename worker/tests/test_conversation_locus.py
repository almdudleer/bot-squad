...
def test_set_and_get_locus(tmp_path):
    ...
    assert CL.get_locus(cfg, "bot-squad", "gu_1") == rec
...
def test_resetting_locus_overwrites(tmp_path):
    """Idempotent — the LATEST inbound message's origin always wins."""
    cfg = _cfg(tmp_path)
...
def test_locus_scoped_per_slug_and_gid(tmp_path):
    ...
    assert CL.get_locus(cfg, "alpha", "gu_1")["chat_id"] == "111"
    assert CL.get_locus(cfg, "beta", "gu_1")["chat_id"] == "222"
    assert CL.get_locus(cfg, "alpha", "gu_2")["chat_id"] == "333"
...
def test_persists_across_reload(tmp_path):
    ...
    assert reloaded["bot-squad:gu_1"]["chat_id"] == "111"
